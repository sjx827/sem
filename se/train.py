...
# 将 pesq 相关的导入完全替换为 sisdr
from models.loss import sisdr_score, phase_losses
from models.discriminator import MetricDiscriminator, batch_sisdr
...
def train(rank, args, cfg):
    ...
    # 关键修正：初始化 best_sisdr 为足够小的负数
    best_sisdr, best_sisdr_step = -999.0, 0
    for epoch in range(max(0, last_epoch), cfg['training_cfg']['training_epochs']):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # 修改：使用 batch_sisdr 替代 batch_pesq
            batch_sisdr_score = batch_sisdr(audio_list_r, audio_list_g, cfg)
            ...
            if batch_sisdr_score is not None:
                # 修改：用 sisdr_score 指导判别器
                loss_disc_g = F.mse_loss(batch_sisdr_score.to(device), metric_g.flatten())
            else:
                loss_disc_g = 0
            ...
            if rank == 0:
                ...
                if steps % cfg['env_setting']['validation_interval'] == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        ...
                        # 修改：评估 SI-SDR 而不是 PESQ
                        val_sisdr_score = sisdr_score(audios_r, audios_g, cfg).item()
                        print('Steps : {:d}, SI-SDR Score: {:4.3f}, s/b : {:4.3f}'.
                                format(steps, val_sisdr_score, time.time() - start_b))
                        
                        sw.add_scalar("Validation/SI-SDR Score", val_sisdr_score, steps)
                    ...
                    # 修改：根据 SI-SDR 找最佳模型 (越大越好)
                    if val_sisdr_score >= best_sisdr:
                        best_sisdr = val_sisdr_score
                        best_sisdr_step = steps
                    print(f"valid: SI-SDR {val_sisdr_score}, Mag_loss {val_mag_err}. Best_SISDR: {best_sisdr} at step {best_sisdr_step}")
